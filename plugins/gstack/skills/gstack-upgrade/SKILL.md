...
```bash
if [ -d "$HOME/.copilot/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.copilot/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".copilot/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".copilot/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".copilot/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".copilot/skills/gstack"
elif [ -d "$HOME/.copilot/skills/gstack" ]; then
...
cd "$INSTALL_DIR" && ./setup --host copilot
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.copilot/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.copilot/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.copilot/skills/gstack"
  fi
fi
_TEAM_MODE=$(~/.copilot/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .copilot/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.copilot/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.copilot/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
cd "$LOCAL_GSTACK" && ./setup --host copilot
rm -rf "$LOCAL_GSTACK.bak"
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.copilot/skills/gstack/` when you're ready."
...
if [ -d "$MIGRATIONS_DIR" ]; then
  for migration in $(find "$MIGRATIONS_DIR" -maxdepth 1 -name 'v*.sh' -type f 2>/dev/null | sort -V); do
    ...
    if [ "$OLD_VERSION" != "unknown" ] && [ "$(printf '%s\n%s' "$OLD_VERSION" "$m_ver" | sort -V | head -1)" = "$OLD_VERSION" ] && [ "$OLD_VERSION" != "$m_ver" ]; then
      ...
      # ~/.gstack state) default to ~/.copilot/skills/gstack when unset —
...
.copilot/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.copilot/skills/gstack/` when you're ready."
